# Histórico de fontes da biblioteca

Este arquivo registra **de onde veio cada release** e **qual método foi usado para montar a versão final**.  
A ideia é manter o mesmo padrão para animes, desenhos, séries e filmes, facilitando refazer uma biblioteca no futuro sem depender de memória.
...
| Uso | Origem |
| --- | --- |
| Episódios / vídeo | **WCOStream** (`wcostream.tv` / `m.wcostream.tv`) |
| Player/entrega intermediária | `embed.wcostream.com` / `getvidlink.php` |

### Método utilizado

1. Os episódios foram obtidos a partir do **WCOStream**.
2. O `video-dl` usa o tratamento específico de WCOStream para resolver o player embed.
3. O fluxo passa pelo endpoint `getvidlink.php` para descobrir a URL temporária do vídeo.
4. O MP4 final é baixado com os headers exigidos pelo servidor.
5. Os arquivos locais usados na biblioteca foram organizados como episódios `SxxExx` e apareceram no fluxo atual como versões **720p**.

### Fontes auxiliares de metadata/títulos

Estas fontes **não são a origem dos arquivos de vídeo**:

- **TMDB:** metadata e títulos de episódio em inglês/fallback;
- **Oggy e as Baratas Tontas Fandom:** apoio para títulos PT-BR quando disponível.

### Resultado / observações

- Fonte de mídia registrada: **WCOStream**.
- Não há, neste histórico, um link individual original de episódio do WCOStream preservado.
- Para reconstrução futura, procurar a mesma obra no WCOStream e usar o fluxo específico já suportado pelo `video-dl`.
...
| Uso | Origem |
| --- | --- |
| Episódios / vídeo | **AnimesDigital** |
| Exemplo de episódio usado | https://animesdigital.org/video/a/112077/ |
| Temporada 1 | https://animesdigital.org/anime/a/coragem-o-cao-covarde-dublado-1a-temporada |
| Player intermediário | `api.anivideo.net/videohls.php` |
| CDN/HLS observada | `cdn-sv01.maximaimg.online/.../index.m3u8` |

### Método utilizado

1. A página do episódio no **AnimesDigital** fornece nome da obra, temporada, episódio e idioma.
2. O player leva a um manifesto **HLS/M3U8**.
3. O `video-dl` resolve o HLS e baixa com **Streamlink**, usando até **8 segmentos em paralelo**.
4. Depois, o FFmpeg faz o remux para MP4 sem re-encode.
5. O arquivo testado da Temporada 1 resultou em **H.264, 640×480 (480p, 4:3)**.
6. A página da temporada pode ser usada com `--series` para descobrir e processar os episódios da temporada em lote.

### Resultado / observações

- Fonte de mídia registrada: **AnimesDigital**.
- Exemplo confirmado: Temporada 1, episódio 2.
- O manifesto observado no teste foi:
  `https://cdn-sv01.maximaimg.online/stream/c/coragem-o-cao-covarde-dublado/02.mp4/index.m3u8`
- O resolvedor HLS agora é genérico no `video-dl`; o adaptador do AnimesDigital fica responsável principalmente por metadata e listagem de episódios.
